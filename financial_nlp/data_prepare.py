...
#            re_str = re_object.subn(u"十一",unicode(row[col_name],'utf-8'))
            re_str = re_object.subn(u"十一", row[col_name].strip())
            # 纠正一些词
...
def pre_train_char_w2v(binary = False):
    data_local_df = pd.read_csv(project.data_dir + train_all, sep='\t', header=None,names=["index", "s1", "s2", "label"],encoding='utf-8')
    data_test_df = pd.read_csv(project.data_dir + test_all, sep='\t', header=None, names=["index", "s1", "s2", "label"],encoding='utf-8')
    # 加载停用词
    stopwords = load_stopwordslist(project.aux_dir + stop_words_path)
    print(stopwords)
    # 加载拼写错误替换词
...
print(project.aux_dir + dict_path)
jieba.load_userdict(project.aux_dir + dict_path)
    
print(project.data_dir+train_all)
data_local_df = pd.read_csv(project.data_dir + train_all, sep='\t', header=None,names=["index", "s1", "s2", "label"])
print(data_local_df['s1'])

print(project.data_dir + test_all)
data_test_df = pd.read_csv(project.data_dir + test_all, sep='\t', header=None, names=["index", "s1", "s2", "label"])

print(project.data_dir + train_data_all)
