...
if __name__ == '__main__':
    ...
    # 提取深度学习特征
    extract_feature_siamese_lstm_manDist() # h.基于LSTM进行语义编码的曼哈顿距离相似度和余弦相似度的模型 
    extract_feature_siamese_lstm_attention() # 基于LSTM进行语义编码的match vector形式计算的相似度的模型 
    extract_feature_siamese_lstm_dssm()
    extract_feature_abcnn()
    
    # 提取NLP特征
    extract_sentece_length_diff()  # a.两个语句的长度上的差距. 
    extract_edit_distance()  # b.两个语句的编辑距离. 
    extract_ngram() # c.两个语句的n-gram相似性的特征. 
    extract_sentence_diff_same()  # d.两个语句的词的统计特征.包括相同词的个数，不同词的个数，Jaccard相似度。
    extract_doubt_sim()  # e.两个语句中的疑问词的相似度.
    extract_sentence_exist_topic()  # f.两个语句是否同时包括蚂蚁花呗和蚂蚁借呗相关的主题.
    extract_word_embedding_sim()  # g.两个语句的词向量组合的相似度.
